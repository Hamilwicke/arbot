...
while True:
    try:
        ...
        if snsr.forward_distance()[0] <= stopping_distance or snsr.forward_distance()[0] == None:
            print 'left sensor proximity warning: making right turn'
            arbot.stop(crusing_speed)
            arbot.right_turn(255,1)
            pass
        if snsr.forward_distance()[1] <= stopping_distance or snsr.forward_distance()[1] == None:
